...
interface CapitalApplicationDetail {
  ...
  purpose: string;
  isCapitalApplication: boolean;
  approvalDate: string;
  approvalAuthority: string;
  remarks: string;
}

interface ItrSevenScheduleECProps {
  capitalApplicationDetails: CapitalApplicationDetail[];
  onAddDetail: () => void;
  onRemoveDetail: (index: number) => void;
  onUpdateDetail: (index: number, field: keyof CapitalApplicationDetail, value: string | number | boolean) => void;
}

const ItrSevenScheduleEC: React.FC<ItrSevenScheduleECProps> = ({
  capitalApplicationDetails,
  onAddDetail,
  onRemoveDetail,
  onUpdateDetail
}) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-red-50 p-4 rounded-lg">
        <h2 className="text-xl font-bold text-red-800 mb-2">Schedule EC</h2>
      ...
      <div className="space-y-4">
        {capitalApplicationDetails.map((detail, index) => (
          <div key={index} className="border border-gray-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-700">Capital Application Detail {index + 1}</h3>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Is Capital Application
                </label>
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    checked={detail.isCapitalApplication}
                    onChange={(e) => onUpdateDetail(index, 'isCapitalApplication', e.target.checked)}
                    className="mr-2"
                  />
                  <span className="text-sm text-gray-700">Is Capital Application</span>
                </div>
              </div>
        ...
        >
          Add Capital Application Detail
        </button>
      </div>
    </div>
  );
...
export default ItrSevenScheduleEC;
