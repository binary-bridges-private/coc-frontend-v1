...
interface StatedObjectApplicationDetail {
...
interface ItrSevenScheduleAProps {
  statedObjectApplicationDetails: StatedObjectApplicationDetail[];
  onAddDetail: () => void;
  onRemoveDetail: (index: number) => void;
  onUpdateDetail: (index: number, field: keyof StatedObjectApplicationDetail, value: string | number | boolean) => void;
}

const ItrSevenScheduleA: React.FC<ItrSevenScheduleAProps> = ({
  statedObjectApplicationDetails,
  onAddDetail,
  onRemoveDetail,
  onUpdateDetail
}) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-yellow-50 p-4 rounded-lg">
        <h2 className="text-xl font-bold text-yellow-800 mb-2">Schedule A</h2>
        <p className="text-sm text-yellow-700">
          Amount applied to stated objects of the trust/institution during the previous year from all sources referred to in C1 to C7 E1 to E7 8 of this table- Revenue Account [to be filled by assessee claiming exemption u/s 11 and 12 or u/s 10(23C)(iv) or 10(23C)(v) or 10(23C)(vi) or 10(23C)(via)]
        </p>
      </div>

      <div className="space-y-4">
        {statedObjectApplicationDetails.map((detail, index) => (
          <div key={index} className="border border-gray-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-700">Stated Object Application Detail {index + 1}</h3>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={detail.sourceOfFunds}
                  onChange={(e) => onUpdateDetail(index, 'sourceOfFunds', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
        ...
        >
          Add Stated Object Application Detail
        </button>
      </div>
    </div>
  );
...
export default ItrSevenScheduleA;
