...
interface ItrSixScheduleVDAProps {
...
const ItrSixScheduleVDA: React.FC<ItrSixScheduleVDAProps> = ({
  vdaDetails,
  onAddDetail,
  onRemoveDetail,
  onUpdateDetail
  ...
  return (
    <div className="space-y-6">
        ...
        {vdaDetails.map((detail, index) => (
          <div key={index} className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="mt-4 bg-gray-50 p-4 rounded-md">
              <h4 className="text-sm font-semibold text-gray-700 mb-2">VDA Transaction Summary</h4>
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        <h3 className="text-lg font-semibold text-gray-700 mb-4">VDA Summary</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div className="bg-white p-3 rounded-md">
            <div className="text-gray-600">Total Transactions</div>
            <div className="text-lg font-bold text-purple-600">{vdaDetails.length}</div>
          </div>
          <div className="bg-white p-3 rounded-md">
            <div className="text-gray-600">Total Sale Value</div>
            <div className="text-lg font-bold text-green-600">
              ₹{vdaDetails.reduce((sum, detail) => sum + detail.salePrice, 0).toLocaleString()}
            </div>
          </div>
          <div className="bg-white p-3 rounded-md">
            <div className="text-gray-600">Total Purchase Value</div>
            <div className="text-lg font-bold text-blue-600">
              ₹{vdaDetails.reduce((sum, detail) => sum + detail.purchasePrice, 0).toLocaleString()}
            </div>
          </div>
          <div className="bg-white p-3 rounded-md">
            <div className="text-gray-600">Net Capital Gain/Loss</div>
            <div className="text-lg font-bold text-purple-600">
              ₹{vdaDetails.reduce((sum, detail) => sum + calculateCapitalGain(detail), 0).toLocaleString()}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 bg-red-50 p-6 rounded-lg">
        ...
        <div className="space-y-2 text-sm text-red-700">
          ...
          <p><strong>Company Specific:</strong> Companies dealing in VDA need to maintain proper records</p>
...
export default ItrSixScheduleVDA;
