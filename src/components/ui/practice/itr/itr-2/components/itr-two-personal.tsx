...
const ItrTwoPersonal: React.FC<ItrTwoPersonalProps> = ({
  onComplete,
  onCancel,
  initialData,
}) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-6">
                  ...
                  .map(([key, error]: [string, any]) => {
                    ...
                    if (Array.isArray(error) && error.length > 0) {
                      return error.map((item: any, index: number) => {
                        if (!item) return null;
                        const nestedErrors = Object.entries(item).filter(
                          ([_, val]) => val
                        );
                        return nestedErrors.map(
                          ([nestedKey, nestedError]: [string, any]) => (
                            <li
                            ...
                            >
                              ...
                              >
                                <span className="font-semibold">
                                  {key === "companyDetails"
                                    ? `Company ${index + 1}`
                                    : key === "equitySharesDetails"
                                    ? `Equity Share Entry ${index + 1}`
                                    : key
                                        .replace(/([A-Z])/g, " $1")
                                        .trim()}{" "}
                                  -{" "}
        ...
        <div className="space-y-4">
          <div>
            ...
            {errors.filedUnderSection && (
              <p className="mt-1 text-xs text-red-500">
                {errors.filedUnderSection.message}
              </p>
            )}
          </div>

          <FilingStatusAdvanced
            register={register}
            watch={watch}
            errors={errors}
            control={control}
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {Object.keys(errors).length > 0
            ? `Fix ${Object.keys(errors).length} Errors`
            : "Save & Continue"}
        </button>
      </div>
    </form>
