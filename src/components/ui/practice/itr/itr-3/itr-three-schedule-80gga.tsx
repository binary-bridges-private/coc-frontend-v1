...
const Schedule80GGASchema = z
  ...
  .superRefine((data, ctx) => {
    // Validate all numeric fields and PAN
    data.donations?.forEach((row, idx) => {
      if (row.panDonee && row.panDonee.trim() !== "") {
        if (!validatePAN(row.panDonee)) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["donations", idx, "panDonee"],
    ...
    console.log("Schedule 80GGA - Details of donations for scientific research", data);
  });

export type Schedule80GGAFormData = z.infer<typeof Schedule80GGASchema>;

interface Schedule80GGAProps {
  initialData?: Partial<Schedule80GGAFormData>;
  onSave: (data: Schedule80GGAFormData) => void;
  onNext: () => void;
  onBack: () => void;
}

export default function ItrThreeSchedule80GGA({
  initialData,
  onSave,
  onNext,
  onBack,
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<Schedule80GGAFormData>({
    resolver: zodResolver(Schedule80GGASchema) as any,
  ...
  const calculations = useMemo(() => {
    ...
    const totalCash = (watched.donations || []).reduce((sum, row) => sum + parseVal(row.donationCash), 0);
    const totalOther = (watched.donations || []).reduce((sum, row) => sum + parseVal(row.donationOtherMode), 0);
    const totalDonation = (watched.donations || []).reduce((sum, row) => sum + parseVal(row.totalDonation), 0);
    const totalEligible = (watched.donations || []).reduce((sum, row) => sum + parseVal(row.eligibleAmount), 0);
  ...
  const onSubmit: SubmitHandler<Schedule80GGAFormData> = (data) => {
    onSave(data);
  };

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="sticky top-0 z-40 bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-indigo-200 px-6 py-4 rounded-t-xl shadow-sm">
        <h1 className="text-2xl font-bold text-indigo-900">
          Schedule 80GGA - Details of Donations for Scientific Research or Rural Development
        </h1>
        <p className="mt-1 text-sm text-indigo-700">
          Applicable to individuals deriving income only from business and profession
        </p>
      </div>
      ...
      <div className="max-h-screen overflow-y-auto px-6 py-4 space-y-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border border-indigo-300 bg-white text-xs">
              <thead className="bg-indigo-200">
                <tr>
                  <th className="border border-indigo-300 px-2 py-2 text-left font-semibold">S. No.</th>
                  <th className="border border-indigo-300 px-2 py-2 text-left font-semibold">Relevant Clause</th>
                  <th className="border border-indigo-300 px-2 py-2 text-left font-semibold">Name & Address of Donee</th>
                  <th className="border border-indigo-300 px-2 py-2 text-left font-semibold">PAN of Donee</th>
                  <th className="border border-indigo-300 px-2 py-2 text-center font-semibold">Donation in Cash</th>
                  <th className="border border-indigo-300 px-2 py-2 text-center font-semibold">Donation in Other Mode</th>
                  <th className="border border-indigo-300 px-2 py-2 text-center font-semibold">Total Donation</th>
                  <th className="border border-indigo-300 px-2 py-2 text-center font-semibold">Eligible Amount of Donation</th>
                </tr>
              </thead>
              <tbody>
                {(watched.donations || []).map((row, idx) => (
                  <tr key={row.id} className="hover:bg-indigo-50">
                    <td className="border border-indigo-300 px-2 py-2">{row.sNo}</td>
                    <td className="border border-indigo-300 px-2 py-2">
                      <Controller
                        name={`donations.${idx}.relevantClause`}
                        control={control}
                        render={({ field }) => (
                          <input
                            {...field}
                    ...
                    <td className="border border-indigo-300 px-2 py-2">
                      <Controller
                        name={`donations.${idx}.nameAddressDonee`}
                        control={control}
                        render={({ field }) => (
                          <input
                            {...field}
                          ...
                          <>
                            ...
                            {errors?.donations?.[idx]?.panDonee && (
                              <p className="text-xs text-red-600 mt-0.5">
                                {errors.donations[idx].panDonee?.message}
                              </p>
                            )}
                          </>
                        )}
                    ...
                    <td className="border border-indigo-300 px-2 py-2">
                      <Controller
                        name={`donations.${idx}.donationCash`}
                        control={control}
                        render={({ field }) => (
                          <input
                            {...field}
                    ...
                    <td className="border border-indigo-300 px-2 py-2">
                      <Controller
                        name={`donations.${idx}.donationOtherMode`}
                        control={control}
                        render={({ field }) => (
                          <input
                            {...field}
                    ...
                    <td className="border border-indigo-300 px-2 py-2">
                      <Controller
                        name={`donations.${idx}.totalDonation`}
                        control={control}
                        render={({ field }) => (
                          <input
                            {...field}
                    ...
                    <td className="border border-indigo-300 px-2 py-2">
                      <Controller
                        name={`donations.${idx}.eligibleAmount`}
                        control={control}
                        render={({ field }) => (
                          <input
                            {...field}
              ...
              <tfoot className="bg-indigo-100 font-bold">
                <tr>
                  <td colSpan={4} className="border border-indigo-300 px-2 py-2">
                    Total Donation
                  </td>
                  <td className="border border-indigo-300 px-2 py-2 text-right">
        ...
        <div className="rounded-xl border-2 border-indigo-900 bg-indigo-100 p-6">
          <h3 className="text-lg font-bold text-indigo-900 mb-4">Total Eligible Deduction u/s 80GGA</h3>
          <div className="bg-white p-6 rounded-lg border-2 border-indigo-300">
            ...
            <p className="text-3xl font-bold text-indigo-700">₹{calculations.totalEligible.toFixed(2)}</p>
          </div>
        </div>
      </div>
