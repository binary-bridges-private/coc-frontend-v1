// ITR-4 Validation Utilities

import { VALIDATION_PATTERNS } from './constants/ItrFourConstants.ts';
...
export const validateItrFourStep = (stepNumber: number, formData: any): { isValid: boolean; errors: any } => {
  const errors: any = {};
  let isValid = true;
  ...
  switch (stepNumber) {
    ...
    case 3: // Business/Profession Details
      if (formData.businessProfessionDetails.length === 0) {
        errors.businessProfessionDetails = 'At least one business/profession detail is required';
        isValid = false;
      }
      break;

    case 5: // House Property Details
      if (formData.housePropertyDetails.length > 0) {
        formData.housePropertyDetails.forEach((property: any, index: number) => {
          if (!validateRequired(property.propertyAddress)) {
            errors[`housePropertyDetails_${index}_propertyAddress`] = 'Property address is required';
            isValid = false;
          }
          if (!validateRequired(property.propertyType)) {
            errors[`housePropertyDetails_${index}_propertyType`] = 'Property type is required';
            isValid = false;
          }
        });
      }
...
export const calculateTotalIncome = (formData: any): number => {
  const businessIncome = formData.businessProfessionDetails.reduce((total: number, bp: any) => {
    return total + (bp.optedForPresumptive ? bp.presumptiveIncome : bp.actualIncome || 0);
  }, 0);
