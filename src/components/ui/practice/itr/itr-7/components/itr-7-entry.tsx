...
import { ITR7SummarySection } from '../itr-7.types.ts';

interface EntryProps {
  sections: ITR7SummarySection[];
  activeSectionId: string;
  onSectionSelect?: (sectionId: string) => void;
}

const statusTone: Record<ITR7SummarySection['status'], string> = {
  'in-progress': 'text-blue-600',
  completed: 'text-green-600',
  pending: 'text-gray-500',
};

const ITR7Entry: React.FC<EntryProps> = ({ sections, activeSectionId, onSectionSelect }) => {
  return (
              ...
              <div className="flex items-start gap-4">
                <div className={`flex h-12 w-12 items-center justify-center rounded-full border text-sm font-medium ${
                  ...
                  {isCompleted ? (
                    ...
                    section.id === 'general-info' ? 'GI' : section.id.toUpperCase().slice(0, 2)
                  )}
                </div>
                <div>
