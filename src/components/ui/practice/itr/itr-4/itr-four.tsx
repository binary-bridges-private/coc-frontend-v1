...
import ItrFourEntry, { ItrFourSection, SectionStatus } from './components/itr-four-entry.tsx';
import ItrFourPartAGeneral, { PartAGeneralFormData } from './components/itr-four-part-a-general.tsx';
import ItrFourPartBGTI, { PartBGTIFormData } from './components/itr-four-part-b-gti.tsx';
import ItrFourPartCDeductions, { PartCDeductionsFormData } from './components/itr-four-part-c-deductions.tsx';
import ItrFourPartDTaxComputations, { PartDTaxComputationsFormData } from './components/itr-four-part-d-tax-computations.tsx';
import ItrFourScheduleBP, { ScheduleBPFormData } from './components/itr-four-schedule-bp.tsx';
import ItrFourVerification, { VerificationFormData } from './components/itr-four-verification.tsx';
...
const ITR_FOUR_SECTIONS: ItrFourSection[] = [
  // Main Parts A, B, C, D
  {
    ...
    description: 'Personal details, address, contact information, and filing status',
    status: 'in-progress',
  },
  {
    id: 'part-b-gti',
    title: 'Part B - Gross Total Income',
    description: 'Income from business/profession, salary, house property, and other sources',
    status: 'not-started',
  },
  {
    id: 'part-c-deductions',
    title: 'Part C - Deductions and Taxable Total Income',
    description: 'Chapter VI-A deductions (80C to 80U) and computation of taxable total income',
    status: 'not-started',
  },
  {
    id: 'part-d-tax-computations',
    title: 'Part D - Tax Computations and Tax Status',
    description: 'Tax liability, interest, fees, and refund/amount payable calculation',
    status: 'not-started',
  },
  ...
  // Schedules (Supporting Details)
  {
    ...
    description: 'Details of income from business or profession under sections 44AD/44ADA/44AE',
    status: 'not-started',
  },  
  {
    id: 'verification',
...
const exportToCSV = (allFormData: any) => {
  ...
  link.setAttribute('download', `ITR4_SUGAM_${new Date().toISOString().split('T')[0]}.csv`);
...
const ItrFour: React.FC = () => {
  const [sections, setSections] = useState<ItrFourSection[]>(ITR_FOUR_SECTIONS);
        ...
        {activeSectionId ? (
          <section id={`section-${activeSectionId}`} className="space-y-4">
            ...
            {activeSectionId === 'part-b-gti' && (
              <ItrFourPartBGTI
                onNext={() => handleSectionComplete('part-b-gti', formData.partBGTI)}
                onBack={handleBackToSummary}
                onSave={(data) => setFormData((prev) => ({ ...prev, partBGTI: data }))}
            ...
            {activeSectionId === 'part-c-deductions' && (
              <ItrFourPartCDeductions
                onNext={() => handleSectionComplete('part-c-deductions', formData.partCDeductions)}
                onBack={handleBackToSummary}
                onSave={(data) => setFormData((prev) => ({ ...prev, partCDeductions: data }))}
            ...
            {activeSectionId === 'part-d-tax-computations' && (
              <ItrFourPartDTaxComputations
                onNext={() => handleSectionComplete('part-d-tax-computations', formData.partDTaxComputations)}
                onBack={handleBackToSummary}
                onSave={(data) => setFormData((prev) => ({ ...prev, partDTaxComputations: data }))}
                initialData={formData.partDTaxComputations}
                taxableTotalIncome={formData.partCDeductions?.c20_taxableTotalIncome || '0'}
              />
            )}
            ...
            {activeSectionId === 'schedule-bp' && (
              <ItrFourScheduleBP
                ...
                onBack={handleBackToSummary}
                onSave={(data) => setFormData((prev) => ({ ...prev, scheduleBP: data }))}
            ...
            {activeSectionId === 'verification' && (
              <ItrFourVerification
                onNext={() => handleSectionComplete('verification', formData.verification)}
                onBack={handleBackToSummary}
                onSave={(data) => setFormData((prev) => ({ ...prev, verification: data }))}
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-6 rounded-lg bg-blue-50 p-4">
                  <p className="text-sm text-blue-900">
                    This section is under development. Click the button below to mark it as complete and proceed.
                  </p>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-2 text-sm text-gray-700">
                      Congratulations! You have successfully completed all sections of your ITR-4 SUGAM return.
